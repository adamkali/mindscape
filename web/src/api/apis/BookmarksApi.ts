...
export class BookmarksApi extends runtime.BaseAPI {
	 ...
	 */
	async createBookmarkRaw(
		requestParameters: CreateBookmarkRequest,
		initOverrides?: RequestInit | runtime.InitOverrideFunction,
	): Promise<runtime.ApiResponse<BookmarkResponse>> {
		...
		const response = await this.request(
			{
				...
				body: RepositoryCreateBookmarkParamsToJSON(
					requestParameters['createBookmarkRequest'],
				),
			},
			initOverrides,
		...
		return new runtime.JSONApiResponse(response, (jsonValue) =>
			BookmarkResponseFromJSON(jsonValue),
		);
	}
	...
	async createBookmark(
		...
		const response = await this.createBookmarkRaw(
			requestParameters,
			initOverrides,
		);
	 ...
	 * Delete a Bookmark
	 */
	async deleteBookmarkRaw(
		requestParameters: DeleteBookmarkRequest,
		initOverrides?: RequestInit | runtime.InitOverrideFunction,
	): Promise<runtime.ApiResponse<BookmarksResponse>> {
		if (requestParameters['authorization'] == null) {
			throw new runtime.RequiredError(
				'authorization',
		...
		const response = await this.request(
			{
				path: `/bookmarks/folder/{bookmark_id}`.replace(
					`{${'bookmark_id'}}`,
					encodeURIComponent(String(requestParameters['bookmarkId'])),
				),
				method: 'DELETE',
				headers: headerParameters,
				query: queryParameters,
		...
		return new runtime.JSONApiResponse(response, (jsonValue) =>
			BookmarksResponseFromJSON(jsonValue),
		);
	}
	...
	async deleteBookmark(
		...
		const response = await this.deleteBookmarkRaw(
			requestParameters,
			initOverrides,
		);
		return await response.value();
	...
	async getBookmarksRaw(
		...
		const response = await this.request(
			{
				path: `/bookmarks/folder/{parent_id}`.replace(
					`{${'parent_id'}}`,
	...
	async getBookmarks(
		...
		const response = await this.getBookmarksRaw(
			requestParameters,
			initOverrides,
		);
		return await response.value();
	 ...
	 * Move a Bookmark
	 */
	async moveBookmarkRaw(
		requestParameters: MoveBookmarkOperationRequest,
		initOverrides?: RequestInit | runtime.InitOverrideFunction,
	): Promise<runtime.ApiResponse<BookmarksResponse>> {
		if (requestParameters['authorization'] == null) {
			throw new runtime.RequiredError(
		...
		const response = await this.request(
			{
				...
				headers: headerParameters,
				query: queryParameters,
				body: MoveBookmarkRequestToJSON(
					requestParameters['moveBookmarkRequest'],
				),
			},
			initOverrides,
		);
	...
	async moveBookmark(
		...
		const response = await this.moveBookmarkRaw(
			requestParameters,
			initOverrides,
		);
		return await response.value();
