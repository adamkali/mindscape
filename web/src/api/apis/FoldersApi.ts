...
export class FoldersApi extends runtime.BaseAPI {
	...
	async createFolderRaw(
	...
	): Promise<runtime.ApiResponse<ResponsesFolderResponse>> {
		...
		const response = await this.request(
			{
				...
				body: RepositoryCreateFolderParamsToJSON(
					requestParameters['createFolderRequest'],
				),
			},
			initOverrides,
		...
		return new runtime.JSONApiResponse(response, (jsonValue) =>
			ResponsesFolderResponseFromJSON(jsonValue),
		);
	}
	...
	async createFolder(
		requestParameters: CreateFolderRequest,
		initOverrides?: RequestInit | runtime.InitOverrideFunction,
	): Promise<ResponsesFolderResponse> {
	...
	async deleteFolderRaw(
		...
		const response = await this.request(
			{
				path: `/folders/{folder_id}`.replace(
					`{${'folder_id'}}`,
					encodeURIComponent(String(requestParameters['folderId'])),
				),
				method: 'DELETE',
				headers: headerParameters,
				query: queryParameters,
	...
	async deleteFolder(
		...
		const response = await this.deleteFolderRaw(
			requestParameters,
			initOverrides,
		);
		return await response.value();
	 ...
	 */
	async getRootFoldersRaw(
		requestParameters: GetRootFoldersRequest,
		initOverrides?: RequestInit | runtime.InitOverrideFunction,
	): Promise<runtime.ApiResponse<ResponsesFoldersResponse>> {
		if (requestParameters['authorization'] == null) {
			throw new runtime.RequiredError(
				'authorization',
		...
		return new runtime.JSONApiResponse(response, (jsonValue) =>
			ResponsesFoldersResponseFromJSON(jsonValue),
		);
	}
	...
	async getRootFolders(
		requestParameters: GetRootFoldersRequest,
		initOverrides?: RequestInit | runtime.InitOverrideFunction,
	): Promise<ResponsesFoldersResponse> {
		const response = await this.getRootFoldersRaw(
			requestParameters,
			initOverrides,
		);
		return await response.value();
	 ...
	 */
	async moveFolderRaw(
		requestParameters: MoveFolderRequest,
		initOverrides?: RequestInit | runtime.InitOverrideFunction,
	): Promise<runtime.ApiResponse<ResponsesFolderResponse>> {
		if (requestParameters['authorization'] == null) {
			throw new runtime.RequiredError(
		...
		const response = await this.request(
			{
				...
				headers: headerParameters,
				query: queryParameters,
				body: RequestsMoveFolderRequestToJSON(
					requestParameters['moveFolderRequest'],
				),
			},
			initOverrides,
