...
const Home = () => {
    ...
    const refreshFolder = async (folderId?: string | undefined) => {
        if (folderId === undefined || folderId === EmptyGuid || folderId === '') {
            await fetchRootFolders();
        } else {
            ...
            try {
                ...
                if (response.success && response.data !== undefined) {
                    // now replace the folder in the list with the new data
                    const data: ResponsesFolderData = response.data;
    ...
    return (
        ...
        >
            ...
            <div class="flex h-screen">
                {/* Sidebar with tree */}
                <div
                    class={`treeview-container m-2 p-4 rounded-3xl overflow-y-auto bg-white/10 backdrop-blur-lg border border-white/20 max-h-[calc(100vh-2rem)] min-w-80 shadow-2xl shadow-slate-900/30 ${isDragOverRoot() ? 'ring-2 ring-blue-400 bg-blue-100/20' : ''
                        }`}
                    ...
                    <div class="p-2">
                        ...
                        <Show when={showCreateFolder()}>
                            <CreateFolderComponent
                                ...
                                refresh={async () => await refreshFolder(focusedNodeId())}
                            />
                        </Show>
